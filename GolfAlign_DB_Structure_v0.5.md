...
3. 영상/이미지는 Google Drive에 먼저 저장하고, NAS에 백업한다.
4. DB에는 파일 자체가 아니라 파일 위치와 메타데이터를 저장한다.
5. 나중에 Supabase, PostgreSQL, S3/R2로 교체하기 쉽도록 컬럼명을 일반화한다.
6. 프로는 회원이 공유한 기록만 볼 수 있다.
7. 프로방 회원 목록은 이름과 프로필 이미지만 공개한다.
8. 다른 회원의 기록, 피드백, 훈련 결과는 비공개다.
